...
for block in children:
    ...
    if (
        texto == "refeições"
        and block["type"] == "to_do"
        and block["to_do"]["checked"] == True
    ):
    ...
    if (
        texto == "refeições"
        and block["type"] == "to_do"
        and block["has_children"] == True
    ):
        notion_children = notion.blocks.children.list(block["id"])["results"]
        for child in notion_children:
            if child["type"] == "to_do" and child["to_do"]["checked"] == True:
                notion.blocks.update(block_id=child["id"], to_do={"checked": False})
                print(
